...
class TestAssertionsDetection {
    ...
    fun setup() {
        ...
        // Se simpleTest/simpleClass sono File:
        val simpleTestFile: String = simpleTest
        val simpleClassFile: String = simpleClass

        testCompilationUnit = parser.parse(simpleTestFile).result
            .orElseThrow { IllegalArgumentException("Parsing failed for test: $simpleTestFile") }

        productionCompilationUnit = parser.parse(simpleClassFile).result
            .orElseThrow { IllegalArgumentException("Parsing failed for source: $simpleClassFile") }
